...
from CROWN_Reach.src.reachability import CT_Dyn_Reach
from CROWN_Reach.src.reachability_baseline import CT_Dyn_Reach_immrax, DT_Plan_Reach_CROWN
...
split_cfg = {0: 2, 1: 2, 2: 2, 3: 2, 4: 2, 5: 2}
# split_cfg = {}
...
for method in methods:
    if method == 'ours':
        reach_analyzer = CT_Dyn_Reach(f_wrapper, state_dim=state_dim, nn_dyn=False, step_size=1/dyn_frequency, 
                                  init_remainder=init_remainder, frr_rounds=frr_rounds, frr_stop_ratio=frr_stop_ratio, sr_window_size=sr_window_size)
    else:
        reach_analyzer = CT_Dyn_Reach_immrax(f_wrapper, state_dim=state_dim, nn_dyn=False, step_size=1/dyn_frequency, 
                                  init_remainder=init_remainder, frr_rounds=frr_rounds, frr_stop_ratio=frr_stop_ratio, sr_window_size=sr_window_size)
    ...
    for T_reach in T_reach_list:
        ...
        if method == 'immrax':
            z_init_lo, z_init_up = prepare_initial_set_v2(z_base_lo, z_base_up, split_cfg)
        else:
            z_init_lo, z_init_up = z_base_lo, z_base_up

        start_time = time.time()
        ts, r_lo, r_up, x_nexts_all, _ = jit_verify(z_init_lo, z_init_up, n_total_steps=T_reach)
        end_time = time.time()
...
out_file = f"experiments/results/reach_ct_dyn_eps{reach_eps}_batch{n_reach_batch}.pkl"
